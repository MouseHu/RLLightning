...
class A2CLearner(BaseLearner):
    """
    PyTorch Lightning implementation of A2C.
    """
    ...
    def __init__(self, args: Namespace, component: Namespace) -> None:
        ...
        assert self.args.eval_freq % self.args.steps_per_epoch == 0, \
            "a2c requires eval freq be the multiple of steps per epoch, otherwise it is never evaluated"
        self.populate(0)
    ...
    def on_after_backward(self) -> None:
        torch.nn.utils.clip_grad_norm(self.agent.parameters(), 0.5, norm_type=2)
    ...
    def add_model_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument("--vf_coef", type=float, default=0.25, help="coefficient for entropy bonus")
        ...
        #NOTE:参考stable_baseline，好像on-policy不会对同一batch多次优化
        parser.add_argument(
            "--nb_optim_iters", type=int, default=1, help="how many steps of gradient descent to perform on each batch"
        )
