...
from env.env.worker import EnvWorker
from env.env.utils import CloudpickleWrapper
...
def _worker(
    ...
    parent.close()#子进程应该不需要访问parent的Pipe
...
class SubprocEnvWorker(EnvWorker):#worker和env其实已经差不多了
    ...
    def __init__(
        ...
        self.process.start()
        self.child_remote.close()#为什么close?因为不应该拿到child的文件描述符。
